...
#define MAX_LIN 282  // tamnho máximo de caracteres de um tweet + 2 (id)
...
int main (int argc, char *argv[]) {
    ...
    while (fgets(linha, MAX_LIN, entrada)) { //lê cada linha do arquivo texto
        ...
        while (palavra != NULL) { //enquanto encontrar palavras no tweet
            ...
            arvore = InsereAVL (arvore, palavra, &ok, &nodos, &comp_index, &rot, id_num);
    ...
    while (fgets(linha, 20, consulta)) {
        ...
        ptNodo = ConsultaAVL(arvore, linha, &comp_cons);
